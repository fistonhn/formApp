...
const Input = ({ type = "text", label, labelClassName, className, endIcon, ...props }: IInput) => {
  return (
    <div className={`grid text-start gap-y-1 ${className}`}>
      {
        label &&
        <label className={`text-sm font-semibold text-black ${labelClassName}`}>{label}</label>
      }
      <div className=" flex items-center border border-black-300 rounded-md  focus:border-primary shadow-md bg-black ">
